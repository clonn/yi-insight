import hexagrams8 from '@/app/data/hexagram_8.json';
import hexagrams64 from '@/app/data/hexagram_64.json';
...
export const getTrigram = (num: number): string => {
  const remainder = num % 8;
  const trigramMapping = ["乾", "兌", "離", "震", "巽", "坎", "艮", "坤"];
  return trigramMapping[(remainder || 8) - 1];
};

export const getHexagram = (lower: string, upper: string): IHexagram | undefined => {
  return hexagrams64.find(h => h.lowerTrigram === lower && h.upperTrigram === upper);
};

export const getBinaryFromTrigram = (trigram: string): string => {
...
export const calculateResult = async ({
  ...
  setResult,
  setAiInterpretation
}: ICalculateParams): Promise<boolean> => {
  try {
    // 計算本卦
    ...
    const changedPosition = parseInt(bianyao) % 6 || 6;
    ...
    if (!changedLowerTrigram || !changedUpperTrigram) {
      throw new Error('無法計算變卦');
    }
