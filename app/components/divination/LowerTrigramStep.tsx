...
interface LowerTrigramStepProps {
  xiagua: string;
  setXiagua: (value: string) => void;
  handleRandomize: (setter: (value: string) => void) => void;
  onNext: () => void;
  onBack: () => void;
}

export function LowerTrigramStep({
  xiagua,
  setXiagua,
  handleRandomize,
  onNext,
  onBack
}: LowerTrigramStepProps) {
  return (
    <motion.div
      key="step2"
    ...
    >
      <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-indigo-100">
        <div className="flex items-center gap-3 mb-6">
          <label className="text-gray-900 font-bold text-lg whitespace-nowrap">下卦：</label>
