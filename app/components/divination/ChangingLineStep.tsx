...
interface ChangingLineStepProps {
  bianyao: string;
  setBianyao: (value: string) => void;
  handleRandomize: (setter: (value: string) => void) => void;
  onCalculate: () => void;
  onBack: () => void;
}

export function ChangingLineStep({
  bianyao,
  setBianyao,
  handleRandomize,
  onCalculate,
  onBack
}: ChangingLineStepProps) {
  return (
    <motion.div
      key="step4"
    ...
    >
      <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-indigo-100">
        <div className="flex items-center gap-3 mb-6">
          <label className="text-gray-900 font-bold text-lg whitespace-nowrap">變爻：</label>
