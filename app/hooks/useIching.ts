...
interface IchingActions {
  setCurrentStep: (step: number) => void;
  setQuestion: (question: string) => void;
  setXiagua: (xiagua: string) => void;
  setShanggua: (shanggua: string) => void;
  setBianyao: (bianyao: string) => void;
...
export const useIching = () => {
  ...
  const [xiagua, setXiagua] = useState<string>("");
  const [shanggua, setShanggua] = useState<string>("");
  const [bianyao, setBianyao] = useState<string>("");
  const [result, setResult] = useState<string>("");
  const [aiInterpretation, setAiInterpretation] = useState<string>("");
  const [currentStep, setCurrentStep] = useState<number>(1);
  const [isCalculating, setIsCalculating] = useState<boolean>(false);

  const handleRandomize = (setter: (value: string) => void) => {
